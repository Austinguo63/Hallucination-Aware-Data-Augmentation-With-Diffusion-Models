...
def run_experiment(examples_per_class: int = 0, 
    ...
    file_ls = os.listdir(synthetic_dir)
    for file in file_ls:
        if file.endswith(".png"):
            ss = file.split("-")
            nme = ss[0]
            idx = int(ss[1])
            label = NAME_LIST_DIC_COCO[nme]
            image = os.path.join(synthetic_dir, file)
            train_dataset.synthetic_examples[idx].append((image, label))
    cutmix_dataset = None
